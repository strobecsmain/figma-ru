...
| Панель свойств, левая панель, тулбар, файловый браузер | переведено по словарю (559 фраз) |
...
## Ассистент на своём API (по желанию)

Отдельная штука в папке `plugin/` — плагин с окном чата, который ходит в **ваш**
API (Anthropic или OpenAI) и строит макеты в Figma. Кредиты Figma Make он не
тратит вообще: её модель в этом не участвует, вы платите только за свои токены.

Что умеет: создавать и править макеты — фреймы, автолейаут, текст, цвета,
скругления. Чего не умеет: воспроизвести превью работающего приложения, как в
Make, — это серверная песочница Figma, плагину она недоступна.

**Как поставить:** Figma → **Плагины → Разработка → Импортировать плагин из
манифеста…** → выбрать `plugin/manifest.json`. Затем в плагине вкладка «Ключ и
модель» → вставить свой ключ → «Сохранить».

Ключ хранится в локальном хранилище Figma на вашей машине. Домены, куда плагину
разрешено обращаться, перечислены в его манифесте: только `api.anthropic.com` и
`api.openai.com`.

**Кнопка в левой панели.** Патч добавляет в рельс Figma (там же, где «Файл» и
«Слои») кнопку «Ассистент». Сама она ничего не рисует — у внедрённого кода нет
доступа к Plugin API, — а нажимает закреплённый плагин; если он не закреплён,
открывает список плагинов. Чтобы закрепить: правый клик на плагине → «Закрепить».

Кнопка встраивается в чужой интерфейс, поэтому при редизайне Figma может просто
исчезнуть — это безопасно, ничего не ломается. Отключить, не снимая перевод:
`localStorage.setItem('figma-ru:no-rail-button','1')` в консоли веб-приложения.
...
(редактор, 559 фраз). Пул-реквесты с дополнениями приветствуются.
